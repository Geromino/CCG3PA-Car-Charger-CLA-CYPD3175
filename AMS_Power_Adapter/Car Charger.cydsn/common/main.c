...
//       pd_internal_cfet_on(0, false);
//    }
     gpio_set_value (RED_LED_GPIO_PORT_PIN, false);
}
else
{
...
//        pd_internal_cfet_off(0, false);
//    }
    gpio_set_value (RED_LED_GPIO_PORT_PIN, true);
     
}

gpio_set_value(GREEN_LED_GPIO_PORT_PIN,!gpio_read_value(GREEN_LED_GPIO_PORT_PIN));
    #endif
...
int main()
{
 ...
 //   pd_remove_internal_fb_res_div();  AMS Design board  not support FB 
    ...
    SW_Tx_UART_Start();
    WEIZ_DEBUG_PRINT(WZ_GENERAL_PURPOSE, "[%s]\r\n", __func__);
 ...
 //   pd_hal_set_vbus_csa_rsense(pd_get_ptr_pwr_tbl(0)->cur_sense_res); AMS Design not use CSP 
    ...
    #else
    #ifdef CYPD3175
        gpio_hsiom_set_config (RED_LED_GPIO_PORT_PIN, HSIOM_MODE_GPIO, GPIO_DM_STRONG, true);
        gpio_hsiom_set_config (GREEN_LED_GPIO_PORT_PIN, HSIOM_MODE_GPIO, GPIO_DM_STRONG, true);
    #endif
#endif
