...
static void call_psrc_ready_cbk(uint8_t port)
{
    ...
    {
#if APP_EVENT_MSG
		WEIZ_DEBUG_PRINT(WZ_APP_EVENT_FLOW, "[%s]Send Ps_rdy\r\n", __func__);	
#endif
...
void app_psrc_tmr_cbk(uint8_t port, timer_id_t id)
{
    ...
    switch(id)
    {
        case APP_PSOURCE_EN_TIMER:
            ...
            {
                #if APP_EVENT_MSG
				WEIZ_DEBUG_PRINT(WZ_APP_EVENT_FLOW, "[%s] APP_PSOURCE_EN_TIMER expired\r\n", __func__);	
                #endif
...
#else
       APP_VBUS_SET_VOLT_P1(select_volt); //AMS Design not support yet the Feature !!
#endif /* NCP_MANAGEMENT */
    }
#if CCG_PD_DUALPORT_ENABLE
    else
...
static void psrc_shutdown(uint8_t port, bool discharge_dis)
{
#if APP_MSG_DEBU_LEVEL_SDK_FLOW
    WEIZ_DEBUG_PRINT(WZ_GEN_SDK_FLOW, "[%s] Begin \r\n", __func__);
#endif
