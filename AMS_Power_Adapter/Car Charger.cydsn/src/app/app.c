...
void app_event_handler(uint8_t port, app_evt_t evt, const void* dat)
{
    ...
    switch(evt)
    {
        ...
        #if APP_MSG_SDK_FLOW
		print_power();			  
        WEIZ_DEBUG_PRINT(WZ_APP_EVENT_FLOW, "[%s] PD port enable (start) completed\r\n", __func__);
        #endif
...
#if APP_MSG_SDK_FLOW
			 print_power();			  
             WEIZ_DEBUG_PRINT(WZ_APP_EVENT_FLOW, "[%s] Type-C attach event\r\n", __func__);
#endif
        ...
        case APP_EVT_TYPE_C_ERROR_RECOVERY:
            ...
            {
#if APP_MSG_SDK_FLOW
                WEIZ_DEBUG_PRINT(WZ_APP_EVENT_FLOW, "[%s] Type-C disconnect(detach) detected.\r\n", __func__);
#endif
    ...
    WEIZ_DEBUG_PRINT(WZ_APP_EVENT_FLOW, "[%s] Type-C attach event\r\n", __func__);
...
#if APP_EVENT_MSG
				    print_power();
                    WEIZ_DEBUG_PRINT(WZ_APP_EVENT_FLOW, "[%s]Type-C AttachWait state entered\r\n", __func__);
#endif        
#if REGULATOR_REQUIRE_STABLE_ON_TIME
            REGULATOR_ENABLE();
#endif /* REGULATOR_REQUIRE_STABLE_ON_TIME */
...
#if APP_EVENT_MSG 
            print_power();
            WEIZ_DEBUG_PRINT(WZ_APP_EVENT_FLOW, "[%s]New PD message received\r\n", __func__);
#endif
        break;
...
#if APP_EVENT_MSG 
            print_power();
            WEIZ_DEBUG_PRINT(WZ_APP_EVENT_FLOW, "[%s]Unexpected high voltage seen on VBus\r\n", __func__);
#endif
     ...
     break;
        ...
        default:
            /* Nothing to do. */
#if APP_SDK_FUNC_MSG
    WEIZ_DEBUG_PRINT(WZ_GEN_SDK_FLOW, "[%s]\r\n", __func__);
#endif
            break;
    }
...
{
#if ( APP_SDK_FUNC_MSG && !APP_SDK_FUNC_MSG_VALUE)
    WEIZ_DEBUG_PRINT(WZ_GEN_SDK_FLOW, "[%s] \r\n", __func__);
#endif
    uint8_t level;
    uint8_t retVal;
...
void print_power(void)
{
        ...
        uint16_t Voltage_PDSS=0,Current_PDSS=0,Watt;
        Voltage_PDSS = pd_hal_measure_vbus(0);
        Current_PDSS = pd_hal_measure_vbus_cur(0);
        Watt=Voltage_PDSS*Current_PDSS;
        WEIZ_DEBUG_PRINT(WZ_RT_POWER, "[%s] W=%2x [mW] U=%2x [mV] I=%2x [mA] \r\n", __func__,Watt,Voltage_PDSS,Current_PDSS);
 #endif

}	

void pdm_print_power(uint8_t port)
{

    
#if APP_POWER_PDM_MSG
    
     const dpm_status_t *dpm_stat = dpm_get_info(port);
     uint16_t dpm_voltage=0,dpm_current=0,dpm_watt=0;
    
    dpm_voltage=dpm_stat->contract.max_volt ;
    dpm_current=dpm_stat->contract.cur_pwr ;
    dpm_watt=dpm_voltage*dpm_current;
    if(dpm_stat -> contract_exist == true)
    	{
            WEIZ_DEBUG_PRINT(WZ_PDM_POWER, "[%s] W=%2X [mW] U=%2X [mV] I=%2X [mA] \r\n", __func__,dpm_watt,dpm_voltage,dpm_current); 
    	}
#endif
